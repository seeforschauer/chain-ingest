...
| `raw.transactions` | `block_number` | B-tree(from_address), B-tree(to_address) | PK: `(block_number, tx_hash)` |
...
|-----------|---------------|
| **Atomic task claiming** | `BLMOVE` — no double-assignment |
...
| **Bounded Redis memory** | Completed-set entries evicted below contiguous PG watermark after each task |
